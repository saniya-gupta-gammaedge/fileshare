...
export async function createShare({ files, options }) {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      const r2Key        = `shares/${shareId}/${relativePath}`
      const mimeType     = parseMime(fileName)

      await uploadToR2(r2Key, file.buffer, mimeType)

      await query(
        `INSERT INTO share_files (share_id, path, name, size, mime_type, r2_key)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [shareId, relativePath, fileName, file.size, mimeType, r2Key]
      )
    })
  )
...
export async function getShare(shareId, password = null) {
  ...
  const filesResult = await query(
    `SELECT path, name, size, mime_type, r2_key
